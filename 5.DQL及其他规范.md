...
- print() 会调用 QuerySet 的 `__repr__` 魔法方法，内部逻辑为每次都重新查询，无法利用惰性查询特性
...
# 写法1
queryset = Xxx.objects.annotate(mark=Value(1, IntegerField())).values('id', 'mark')[:10]
list(queryset)

# 写法2
queryset = Xxx.objects.values('id', mark=Value(1, IntegerField()))[:10]
list(queryset)
...
queryset = Xxx.objects.annotate(yyy_exists=Exists(exists_qs)). \
    values('id', 'xxx_name', 'yyy_exists')[:10]
list(queryset)
...
exists_qs = Yyy.objects.filter(is_deleted=0, xxx_id=OuterRef('xxx_id')).values('id')
queryset = Xxx.objects.annotate(yyy_exists=Exists(exists_qs)).filter(yyy_exists=True). \
...
subquery = Xxx.objects.annotate(yyy_ctime=Subquery(yyy_sub, DateTimeField())).filter(is_deleted=0).values('xxx_id', 'yyy_ctime')[:10]
list(subquery)
...
### 13.如果小序列数据来自于数据库，且仅作为in子句使用，推荐直接使用 `in子查询`
...
### 14.禁止in超大序列 filter(column__in=huge_sequence)，考虑改写成exists语句

- 超大序列会增加SQL的长度，增加网络I/O，MySQL也可能无法使用索引
- exists语句，对于被驱动表，查询到一行复合条件即返回，性能极佳


### 15.禁止对日期类型字段使用 `__year`，`__month`，`__day`
...
### 18.NULL值的判断，禁止使用 column=None，推荐使用 `__exact`
...
### 19.已知n个（通常5-10个）id，每个id取m条（通常小于5）数据的优化思路
